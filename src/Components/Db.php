...
class Db {
	...
	public function multiQuery (callable $fn) {
		$this->_sql = 'select ';
	}
	...
	public function add ($col, $val, $ignore = false) {
		if ($this->_error) return $this;
		$p = array_fill(0, count($val), '?');

    ($ignore) ? $in = 'insert ignore into ' : $in = 'insert into ';
		if (!is_array(next($val))) {
			$this->_sql = $in . $this->_table .' (' . $this->safe_str($col) .') values(' . implode(', ', $p) . ')';
			$this->_query_value = $val;
		} else {
			$this->_sql = $in . $this->_table .' (' . $this->safe_str($col) .') values ';
			$toAppendToQuery = '';
      foreach ($val as $k => $v) {
				$p = array_fill(0, count($v), '?');
				$toAppendToQuery .= '('. implode(', ', $p) .')';
				if (count($val) - 1 > $k) $toAppendToQuery .= ', ';
			}
			$this->_sql = $this->_sql . $toAppendToQuery;
			$this->_query_value = Utils::arr_flat($val);
		}

		return $this;
	}

	public function insert (array $columns, array $values, bool $ignore = false) {
		$this->add($columns, $values, $ignore);
		return $this;
	}
	...
	public function pages ($ppage = 5, $each = 1) {
		if ($this->_error) return $this;
		...
		if ($each) {
			...
			// reset $each if greater than $last
			if ($last == 1) $each = 1;
			else {
				if ($each < $last) {
					$next = $each + 1;
	...
	public function countRow (string $col = '*', string $alias = 'all') {
	  $this->_sql .= 'select count(`'.$col.'`) as `'.$alias.'` from '. $this->_table;
	  return $this;
	}

	public function countDistinct (string $col = '*', string $alias = 'all') {
	  $this->_sql .= 'select count(distinct `'.$col.'`) as `'.$alias.'` from '. $this->_table;
	  return $this;
	}
    ...
    $queryResult = $this->_result;
		...
		if ($this->_error) error($this->_error);
		$this->toDefault();
		if (stristr($querystring, 'insert')) return $this->_lastid;
		if ($f == false || !$this->count()) return $queryResult;
	...
	public function verbose () {
		verbose($this->_sql, $this->_query_value);
	}
	...
	public function __tostring() {
		return $this->_sql . '<br>' . Utils::json($this->_query_value);
	}
