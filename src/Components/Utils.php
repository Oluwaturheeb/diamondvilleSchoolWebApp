...
class Utils
{
	...
	*/
	...
	public static function multiToAssoc (array $array) {
		if (!is_array(next($array))) error ('Utils::multiToAssoc expects a multidimensional array as arg!');
	...
	public static function collection($src, ...$col)
	{
	  if (!$src) return;
    $cols = [];
    foreach ($col as $c) {
      if (is_array($c)) $cols[$c[0]] = [];
      else $cols[$c] = [];
        ...
        $key = array_keys((array)$val);
				if (is_array($c)) {
				  foreach ($c as $cc) {
				    if (in_array($cc, $key)) {
  				    $val = (array) $val;
  				    if ($val[$cc]) array_push($cols[$c[0]],$val[$cc]);
  				  }
				  }
				} else {
				  if (in_array($c, $key)) {
  				  $val = (array) $val;
  				  if ($val[$c]) array_push($cols[$c],$val[$c]);
  				}
				}
			}
    }
